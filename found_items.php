...
$sql = "SELECT r.report_id, r.item_name, r.description, r.location, r.condition_status, r.created_at, c.category_name
        FROM reports r
        LEFT JOIN categories c ON c.category_id = r.category_id
        WHERE r.type = 'found' AND r.is_public = 1";
$params = [];
...
<head>
<meta charset="UTF-8">
<title>Found Items - Foundly</title>
...
<div class="browse-wrap">
  <h1>Found Items</h1>
  <p class="page-sub">Items reported found around campus. Contact details are never shown here — they're only released to the relevant parties after admin verification.</p>

  <form method="GET" action="found_items.php" class="browse-filters">
  ...
  <?php if (empty($items)): ?>
    <p class="empty-msg">No found items match your search.</p>
  <?php else: ?>
      ...
      <?php foreach ($items as $item): ?>
        <div class="browse-card">
          ...
          <div class="browse-tags">
            <span class="tag"><?php echo htmlspecialchars($item['location']); ?></span>
            <?php if ($item['condition_status']): ?><span class="tag"><?php echo htmlspecialchars(ucfirst($item['condition_status'])); ?></span><?php endif; ?>
          </div>
        </div>
      <?php endforeach; ?>
    </div>
