...
$old = [
    ...
    'found_date' => '', 'condition_status' => 'new', 'location' => '', 'notes' => ''
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '') {
        $stmt = $pdo->prepare(
            'INSERT INTO reports
                (user_id, type, item_name, category_id, description, condition_status,
                 location, notes, date_reported, status, is_public, photo_data, photo_type)
             VALUES
                (?, "found", ?, ?, ?, ?, ?, ?, ?, "open", 1, ?, ?)'
        );
        $stmt->execute([
...
<p class="page-sub">Help reunite a student with their lost property by providing accurate details.</p>
  ...
  <div class="form-grid">
    ...
    <div class="form-card">
      <div class="field-row">
        <div class="field">
          ...
          <input type="text" id="item_name" name="item_name" placeholder="e.g. Silver MacBook Air" value="<?php echo htmlspecialchars($old['item_name']); ?>">
        </div>
        <div class="field">
      ...
      <div class="field">
        ...
        <textarea id="description" name="description" rows="4" placeholder="Describe the item, including any distinguishing features (stickers, scratches, colors)..."><?php echo htmlspecialchars($old['description']); ?></textarea>
      </div>
      ...
      <div class="field">
        ...
        <input type="text" id="notes" name="notes" placeholder="Any extra information for the office staff..." value="<?php echo htmlspecialchars($old['notes']); ?>">
      </div>

      <button type="submit" class="btn btn-navy btn-block">Submit Found Report</button>
    </div>
    ...
    <div>
      ...
      <div class="notice-card green">
        ...
        Found items are kept securely at the central office. Your contact information is only shared with university administrators for verification.
      </div>
      ...
      <div class="form-card tips-card">
        ...
        <ul>
          ...
          <li>Mention unique stickers or marks</li>
          <li>Note the exact time found</li>
        </ul>
      </div>
    </div>
